...
export function Hero() {
  return (
    <section id="top" className="relative min-h-screen overflow-hidden text-ivory">
      ...
      {/* Stats bar */}
      <div className="absolute bottom-0 inset-x-0 z-10 border-t border-ivory/15 bg-charcoal/40 backdrop-blur-md">
        <div className="container-x grid grid-cols-2 md:grid-cols-4">
          {stats.map((s, i) => (
            <div
              key={s.label}
              className={`py-6 lg:py-8 ${i > 0 ? "md:border-l border-ivory/10" : ""} ${i === 2 ? "border-l border-ivory/10" : ""} ${i % 2 === 1 ? "border-l border-ivory/10" : ""}`}
            >
